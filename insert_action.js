...
const main = async () => {
  ...
  // Tạo hành động xử lý khi phím 5 được nhấn
  ...
  /*************************Done create_new_action.js*************************/

  // Tạo hành động khi nhấn phím 2
  const pressKey2Action = await createInteractiveMenuAction({
    scenarioId: scenario.id,
    label: "Phím 2",
    type: "INTERACTIVE_MENU",
    contentType: "TEXT",
  ...
  await updateScenarioAction({
    ...
    contentType: "TEXT",
    contentText:
      "Xin chào quý khách, đây là tổng đài CSKH nhân tạo, Mời bạn bấm phím 2 để nhận tư vấn bán hàng",
    responses: [{ keyPadMatch: "2", nextActionId: pressKey2Action.id }],
  });
